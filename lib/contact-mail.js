...
// Absendername (Anzeigename) fuer alle Anfrage-Mails. Die Adresse kommt weiter aus
// RESEND_FROM/SMTP_FROM (Env); nur der angezeigte Name wird hier gesetzt, damit die
// Mail unter der Marke erscheint statt unter der nackten Adresse. Ueber die Env
// MAIL_FROM_NAME uebersteuerbar.
const MAIL_FROM_NAME = process.env.MAIL_FROM_NAME || "Dolomites Wedding & Elopement Photographer";
const bareAddress = (value) => {
  const match = String(value || "").match(/<([^>]+)>/);
  return (match ? match[1] : String(value || "")).trim();
};
const withFromName = (value) => {
  const addr = bareAddress(value);
  return addr ? `${MAIL_FROM_NAME} <${addr}>` : String(value || "");
};

const sendDirectToRecipientMx = async ({ to, subject, html, text, replyTo, from, fromName }) => {
  ...
  const result = await transporter.sendMail({
    from: fromName ? `${fromName} <${bareAddress(safeFrom) || safeFrom}>` : withFromName(safeFrom),
    to,
...
const sendMail = async ({ to, subject, html, text, replyTo, fromName }) => {
    ...
    try {
      if (transport.type === "resend") {
        const result = await transport.client.emails.send({
          from: fromName ? `${fromName} <${bareAddress(transport.from) || transport.from}>` : withFromName(transport.from),
          to,
          replyTo: normalizedReplyTo,
          subject,
          html,
...
const sendContactInquiry = async ({ rootDir, payload, meta = {} }) => {
  ...
  if (!name || !email || !date || !message) {
    throw new Error("Missing required contact fields: name, email, date or message");
  }

  const subjectDate = date || (lang === "en" ? "Preferred date open" : "Wunschdatum offen");
  const subjectPrefix =
    source === "hochzeitsfotograf.tirol"
      ? (lang === "en" ? "New Inquiry – Hochzeitsfotograf Tirol" : "Neue Anfrage – Hochzeitsfotograf Tirol")
      : source === "blitzkneisser-hub"
      ? (lang === "en" ? "New Inquiry – Dolomiten Wedding Photographer" : "Neue Anfrage – Dolomiten Wedding Photographer")
      : (lang === "en" ? "Your story in the mountains" : "Eure Geschichte in den Bergen");
  const subject = [subjectPrefix, subjectDate].join(" | ");

  const fromName =
    source === "hochzeitsfotograf.tirol"
      ? "Hochzeitsfotograf Tirol"
      : "Blitzkneisser Dolomiten Wedding and Elopement Photographer";
